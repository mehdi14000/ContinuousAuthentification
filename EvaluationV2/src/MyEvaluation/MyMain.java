...
public final class MyMain {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException, FileNotFoundException, InterruptedException{
	     ...
	     while(index != users.size() && index1 != usersMalware.size()){
	    	 ...
	    	 list1=Enrollement.enrollement(list1,featuresUser.get(users.get(index)), hashFunctionUser,secretKeyUser);
	    	 list2=Enrollement.enrollement(list2,featuresMalware.get(usersMalware.get(index1)),hashFunctionMalware,secretKeyMalware);
	    	 ...
	    	 for(int user:users ){
	    		 ...
	    		 do{
	    			 if ((feat.getTimestamp()< currentTime + SLOT_TIME)) {
	    				 ...
	    				 System.out.println(verifier.getTrustScore().getScore());
